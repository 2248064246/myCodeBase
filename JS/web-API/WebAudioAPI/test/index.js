...
let visualSelect = document.querySelector('#visual');
function init() {
  ...
  // .then(res => res.body).then(stream => MediaStreamToWebm.DecodedStream())
    ...
    .then((buffer) => {
      ...
      try {
        audioCtx.decodeAudioData(buffer, (audio) => {
          ...
          let audioSource = new SoundCloudAudioSource(audio);
          visualizer.init({
...
var Visualizer = function () {
  ...
  Polygon.prototype.calculateOffset = function (coords) {
    ...
    var mentalFactor = Math.min(
      Math.max(Math.tan(audioSource.volume / 6000) * 0.5, -20),
      2
...
var SoundCloudAudioSource = function (buffer) {
  var self = this;
  var analyser;
